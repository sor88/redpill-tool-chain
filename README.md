...
Теперь инструкция сводиться к:
1. Распаковываем в удобное место
2. В файле <platform>_user_config.json правим vid, pid, sn, mac
3. 
chmod +x redpill_tool_chain.sh
4. 
./redpill_tool_chain.sh build <platform>-7.0.1-42218
5.
./redpill_tool_chain.sh auto <platform>-7.0.1-42218
6. В папке images будет образ загрузчика, пишем любым удобным способом на флешку. В папке cache будет pat фай с нужной версией DSM для установки в мастере.